...
export const likeOrDislike = async (
    ...
    const response = await backendRequest(
        `environment/${environmentId}/like`,
        "GET"
    );
    return response.ok;
};
...
export const addRuleToEnv = async (
    ...
    const response = await backendRequest(
        `environment/${environmentId}/rules/add`,
        "POST",
        {
            ruleId,
        }
...
export const deleteRuleFromEnv = async (
    ...
    const response = await backendRequest(
        `environment/${environmentId}/rules/${ruleId}`,
        "DELETE"
    );
    return response.status;
};
...
export const getTrendingEnvironments = async (
    skip = 0,
    take = 10
): Promise<backendResponse<environment[]>> => {
    const response = await backendRequest(
        `environment/trending?skip=${skip}&take=${take}`,
        "GET"
    );
    return {
        status: response.status,
...
export const saveEnvironment = async (
    ...
    const response = await backendRequest(
        `environment/${environmentId}/save`,
        "GET"
    );
    return response.ok;
};

export const getSavedEnvironments = async (): Promise<
